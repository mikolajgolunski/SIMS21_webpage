...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if(!empty($message)){
    ...
    elseif ($values["password"] != $values["re_password"]) {
      ...
      $sql = "SELECT COUNT(*) FROM `people` WHERE email = ? LIMIT 1";
      $stmt = $conn->prepare($sql);
      ...
      if($users_count > 0 or $email_count > 0) {
        ...
        $message[] = "person_id=".$conn->lastInsertId();
        $message[] = "create_time=NULL";
        $message = implode(", ", $message);
        $sql = $sql.$message;
        $stmt = $conn->prepare($sql);
        ...
        $sql = "SELECT id, last_name, first_name, middle_name, full_name, title, email, affiliation_id FROM `people` WHERE id=?";
        $stmt = $conn->prepare($sql);
        ...
        $stmt = $conn->prepare($sql);
        $stmt->execute(array($person_data["affiliation_id"]));
