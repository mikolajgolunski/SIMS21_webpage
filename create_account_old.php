...
function send_mails($user_info) {
  ...
  $mail->Subject = "New account created for: ".$user_info["username"].": ".$user_info["full_name"];
  $mail->Body = "There is a new account created on the website for: ".$user_info["username"]."\nName: ".$user_info["full_name"]."\nAffiliation1: ".$user_info["affiliation1"];
  if(!$mail->send()) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if(!empty($message)){
    ...
    elseif ($values["password"] != $values["re_password"]) {
      ...
      $sql = "SELECT COUNT(*) FROM `people` WHERE email = ? AND (type != 'author' OR type IS NULL) LIMIT 1";
      $stmt = $conn->prepare($sql);
      $stmt->execute(array($values["email"]));
      $email_count = $stmt->fetchColumn();
      
      if($users_count > 0 or $email_count > 0) {
        $err = "Username or email already present in the database. Please choose another username/email or login to the existing account.";
        $values["username"] = "";
        $values["email"] = "";
        ...
        $txt = $person_data["full_name"]." (user_id: ".$participant_data["id"].", person_id: ".$person_data["id"].") created new account.";
        log_save($conn, "create_account", $txt);
      ...
      ?>
            ...
            <div id="content">
              ...
              <form id="account" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'])."#openModal" ?>" method="post">
                ...
                <input class="button important" name="register" id="register_button" value="Create account" type="submit">
              </form>
              ...
              <div id="openModal" class="modalDialog">
                <div>
                  <?php
                  if ($err == ""){
                    echo "<p>Your account has been created. You will be redirected to the login page. Please login to access personalized part of the website.<br>If you do not get redirected please click <a href=\"login.php\">here</a>.</p>";
                  } else {
